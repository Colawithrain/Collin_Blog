...
exports.default = pruneNonCriticalCss;
...
function pruneNonCriticalCss({
  ...
  function translateSelectorListRuleToStrings(selectorListRuleObject) {
    return selectorListRuleObject.prelude.children.map(translateSelectorObjectToString);
  }
  ...
  function isCssSelectorRuleCritical(rule) {
    ...
    const criticalSelectorsLength = criticalSelectorsMeta.filter(s => s.toKeep).length;
    const keepRule = criticalSelectorsLength > 0;
    if (keepRule && criticalSelectorsLength < selectors.length) {
      // NOTE: mutating the rule here
      const selectorListFiltered = rule.prelude.children.filter((rule, index) => {
  ...
  function isCssRuleCritical(rule) {
     ...
     */
    if (rule.type === 'Atrule' && (rule.name === 'charset' || rule.name === 'import' || rule.name === 'namespace')) {
      return true;
    }
    ...
    */
    if (rule.type === 'Atrule' && (rule.name === 'font-face' || rule.name === 'keyframes' || rule.name === 'viewport' || rule.name === '-ms-viewport')) {
      return true;
    }
    ...
    */
    ...
    rule.name === 'media' || rule.name === 'document' || rule.name === '-moz-document' || rule.name === 'supports')) {
